...
import { rotaSchema } from '../others/schemas/rota.schema';
import {
...
export function validarDivergenciaConversao(
  shipments: ShipmentPickingMapItem[],
  products: ProductsPickingMapItem[],
): DivergenciaConversao[] {
  ...
  for (const shipment of shipments) {
    ...
    if (product) {
      ...
      divergenciaConversao.set(shipment.codItem, {
        ...
        unidadesNaCaixa: product.unPorCaixa,
        lote: shipment.lote,
        codItem: shipment.codItem,
        descricao: shipment.descricao,
        quantidadeFinal: Math.round(conversao),
        pesoPorUnidade,
        pesoPedido,
        quantidade,
        unMedida,
  ...
  return Array.from(divergenciaConversao.values());
}